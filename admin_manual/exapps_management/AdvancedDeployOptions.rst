...
AppAPI allows optionally to configure environment variables, mounts and port bindings for the ExApp container.
...
By default there are only mounts and ports bindings available for configuration.
...
Docker port bindings can be used to expose the ExApp container ports to the host.
Can be useful in complex network configurations, or specific use cases, when you need to have access
to service inside ExApp container on the host machine or further in the network.

``--port HOST_PORT;CONTAINER_PORT/PROTOCOL or --port HOST_IP;HOST_PORT;CONTAINER_PORT/PROTOCOL``
